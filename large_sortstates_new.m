...
t3(totpi,numorb+2) = t4(:,numorb+2);
fprintf('t4 done\n')


totpi = zeros(size(t3,1),1);
if ~isempty(t3)
    for i = 1:size(t3,1)
        t3si = repmat(t3(i,1:numorb+1),size(t2,1),1);
        diff = abs(sum(t2(:,1:numorb+1)-t3si,2));
...
t2(totpi,numorb+2) = t3(:,numorb+2);
fprintf('t3 done\n')


totpi = zeros(size(t2,1),1);
if ~isempty(t2)
    for i = 1:size(t2,1)
        t2si = repmat(t2(i,1:numorb+1),size(t1,1),1);
        diff = abs(sum(t1(:,1:numorb+1)-t2si,2));
...
t1(totpi,numorb+2) = t2(:,numorb+2);
fprintf('t2 done\n')


totpi = zeros(size(t1,1),1);
if ~isempty(t1)
    for i = 1:size(t1,1)
        t1si = repmat(t1(i,1:numorb+1),size(t0,1),1);
        diff = abs(sum(t0(:,1:numorb+1)-t1si,2));
